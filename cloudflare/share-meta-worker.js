...
async function loadMeta(url, env) {
  ...
  if (parsed.threadRef) {
    if (isHex32(parsed.threadRef)) {
      params.set('thread_id', parsed.threadRef);
    } else {
      params.set('thread_slug', parsed.threadRef);
    }
  }
...
function renderHtml(meta, requestUrl) {
  ...
  const canonical = escapeHtml(
    meta.canonical_url || `${SITE_ORIGIN}${requestUrl.pathname}`,
  );
  ...
  <main>
    ...
    <nav class="actions">
      <a
        class="primary"
        id="open-app"
        href="app.chaput://${escapeHtml(deepPath)}"
      >
        Open in Chaput
      </a>

      <a class="secondary" id="store-link" href="${IOS_STORE}">
        Get Chaput
      </a>
    </nav>
  </main>
    ...
    (function () {
      ...
      if (isAndroid) {
        var intent =
          "intent://chaput.app/" +
          cleanPath +
          "#Intent;scheme=https;package=com.goktigin.chaput;S.browser_fallback_url=" +
